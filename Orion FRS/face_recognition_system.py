# -*- coding: latin-1 -*-
# Face_Recognition_System v1.5.5 by erick esau martinez
...
class Scanner:
    ...
    # start the camera at index 0 and search for persons.
    myscan.scan_camera(camera=0)
    while True:
        ...
        if found:
            print(found)
            myscan.stop() # stop the process close camera
    .
    .
...
"""
    ...
    def delete_profile(self, path):
        """
Delete a profile
delete_profile(profile path)
"""
        # delete encoding and profile path from the lists
        index = self.nombres_conocidos.index(path)
        self.encodings_conocidos.pop(index)
        self.nombres_conocidos.pop(index)
        # delete all files
        l = os.listdir(path)
        for file in l:
            try:
                ...
                print(f'error cant delete {path}/{file}')
        # delete empty folder
        os.rmdir(path)
...
if __name__ == "__main__":
    ...
    print("Person found the profile path is: ", a)
