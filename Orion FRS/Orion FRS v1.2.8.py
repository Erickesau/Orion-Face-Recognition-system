# Orion Face Recognition System.   GUI v1.2.8  
...
from tkinter import Label,Frame,Entry,INSERT,Checkbutton,Spinbox,Button,PhotoImage,Tk,filedialog,Toplevel,ttk,scrolledtext,BooleanVar,IntVar,Listbox,StringVar
...
class Root(Tk):
    def __init__(self):
        ...
        ### -------------  background labels --------------------------
        self.bg1 = Label(self.list_frame, image=self.bg_img, width=1260, height=680)
        self.bg2 = Label(self.search_frame, image=self.bg_img, width=1260, height=680)
        self.bg3 = Label(self.add_frame, image=self.bg_img, width=1260, height=680)
        self.bg1.grid(column=0, row=0)
        self.bg2.grid(column=0, row=0)
        self.bg3.grid(column=0, row=0)
        ...
        Button(self.bar_menu, text="SEARCH FOR FACES", command=self.search, bg="#005588" ,fg="white", font=("Verdana",12), activebackground="#00ccff").place(x = 140, y = 0)
        Button(self.bar_menu, text="ADD FACES", command=self.add, bg="#005588",fg="white",font=("Verdana",12), activebackground="#00ccff").place(x = 330, y = 0)
        Button(self.bar_menu, text="SETTINGS", command=self.settings, bg="#005588", fg="white",font=("Verdana",12), activebackground="#00ccff").place(x = 580, y = 0)
        Button(self.bar_menu, text="X", command=self._exit, bg="red",fg="white", width=4, font=("Verdana",12),relief="raised",borderwidth=5, activebackground="orange").place(x = 1220, y = 0)
        Button(self.bar_menu, text="About", command=self.about, bg="#005588", fg="white",font=("Verdana",12), activebackground="#00ccff").place(x = 680, y = 0)
        ...
        self.bt_open_file.place(x=360, y=270)
        #button close hided
        ...
        Button(self.search_frame, text="      Log      ", command=lambda x=None:(self.log_frame.place(x=83, y=15),self.soundeffect("click")), bg="#00aaff", activebackground="#00ffff", fg="white").place(x=940,y=330)
    ...
    def settings(self):
        ...
        self.dialogo.geometry("300x200+500+200")
        ...
        f = Frame(self.dialogo,bg="#004444",width=300,height=200,relief="groove",borderwidth=10)
        f.place(x=0,y=0)

        # sound checkbox, stop if person found checkbox 
        sound = BooleanVar()
        stop = BooleanVar()
        hud = BooleanVar()
        vid = IntVar()
        ...
        Checkbutton(f, text="Sound Effects           ", var=sound, bg="#00aaff", width=20,relief="raised",borderwidth=5).place(x=20,y=10)
        Checkbutton(f, text="Stop If Person Found",var=stop, bg="#00aaff", width=20,relief="raised",borderwidth=5).place(x=20,y=40)
        Checkbutton(f, text="Show Camera Hud   ",var=hud, bg="#00aaff", width=20,relief="raised",borderwidth=5).place(x=20,y=70)
        ...
        def save():
            ...
            setting = {"sound":sound.get(), "auto_off_camera":stop.get(), "video_source":vid.get(),"show_camera_hud":hud.get()}
            setting = json.dumps(setting)
            with open("setting.json", "w") as f:
                f.write(setting)
            self.dialogo.destroy()
            
        Button(f, text="Ok",command=save, bg="orange",relief="raised",borderwidth=5,activebackground="green").place(x=20,y=150)
        Button(f, text="Cancell",command=lambda x=None:(self.dialogo.destroy(),self.soundeffect("click")), bg="orange", relief="raised",borderwidth=5,activebackground="red").place(x=150,y=150)
    ...
    def file(self):
        ...
        n = filedialog.askopenfile(title="Open image to scan",filetypes=("Images *.jpg","Images *.jpeg","Images *.png"))
        if n:
            ...
            self.lphoto.update()
            # finally send the original img to the face recognition system and it will return the result
            person = myscan.scan_file(n.name)
            # if var person is not false send the name to the show_person_info funcion
            if person:
                self.show_person_info(person, add_to_log=True)
            else:
                # if not person found call message funtion to show message
                self.message(text="Not Person Found")
    ...
    def show_person_info(self, person, wanted_alert=True, add_to_log=False):
        ...
        try:
            ...
            img = Image.open(person+"/"+"image.jpeg")
            # get image size
    ...
    def edit_profile(self):
        ...
        self.new_picture_path = self.last_person_detected +"/"+ "image.jpeg"
    ...
    def message(self, text="", font="arial 20", height=6, bg="red", fg="white"):
        self.msg = Toplevel()
        self.msg.geometry("300x200+500+300")
        self.msg.wm_attributes("-alpha",0.6)
        self.msg.overrideredirect(True)
...
window.title("Face Recognition System V 1.2.8")
